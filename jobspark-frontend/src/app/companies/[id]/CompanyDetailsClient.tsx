...
export default function CompanyDetailsClient({ id }: { id: string }) {
  ...
  const aboutText = company.description || `Welcome to ${company.name}. We are a leading force in the ${company.industry} industry, driven by innovation and excellence. Our dedicated team works tirelessly to create solutions that meet the evolving needs of our customers. Join us as we continue to shape the future of ${company.industry}.`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-10">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 md:p-8 flex flex-col md:flex-row gap-6 items-start justify-between border border-gray-100">
          <div className="flex gap-6 items-center w-full md:w-auto">
            <div className="w-24 h-24 md:w-32 md:h-32 bg-gray-900 rounded-2xl flex-shrink-0 flex items-center justify-center text-white text-4xl font-bold shadow-lg border-4 border-white">
              {company.logo ? (
                    ...
                    <div key={recruiter.id} className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full bg-blue-100 overflow-hidden">
                        {recruiter.user?.image ? (
                          <img src={recruiter.user.image} alt={recruiter.user.name} className="w-full h-full object-cover" />
                        ) : (
